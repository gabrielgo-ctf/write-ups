...
```
8. But the way to send this is very strange. First of all we have to send "\*". After that, it doesn't store the bytes sent... the board counts the number of data received and store this counter when there is a long time without receive more bytes. Finally we have to write another "\*". Example:
   - send "\*"
...
9. After some hours sending (and the board processing) the password. We get the flag 
```
Starting...
