...
#define RESET 5 //To RST of riscurino
#define CS 6 //To D9 of riscurino
#define SI 7 //To D10 of riscurino
#define CLK 8 //To PIN 17 of AT
...
void sendByteHackSPI(byte v)
{
  for (int i = 0; i < 8; i++)
  {
    digitalWrite(CLK, LOW);
    digitalWrite(SI, (((v >> (7 - i)) & 1) > 0) ? HIGH : LOW);
    digitalWrite(CLK, HIGH);
  }
}

void sendBytesHackSPI(byte *data, int c)
{
  digitalWrite(CS, LOW);   
  for (int i = 0; i< c; i++)
  {
    sendByteHackSPI(data[i]);
    delayMicroseconds(2);
  }
...
void loop()
{
  if (ID == 0x7D3)
  {
    ...
    pinMode(CS, OUTPUT);
    pinMode(CLK, OUTPUT);
    byte cmd1[] = {0x05, 0x0F, 0xE0, 0x80};
    byte cmd2[] = {0x02, 0x10, 0xFA, 0x60, 0x00, 0x00};
    byte cmd3[] = {0x02, 0x24, 0xFF, 0xE0, 0x00, 0x00};
    byte cmd4[] = {0x05, 0x0F, 0xE0, 0x00};
    sendBytesHackSPI(cmd1, 4);
    sendBytesHackSPI(cmd2, 6);
    sendBytesHackSPI(cmd3, 6);
    sendBytesHackSPI(cmd4, 4);
    pinMode(SI, INPUT);
    pinMode(CS, INPUT);
    pinMode(CLK, INPUT);
  }
  ...
  */
  ...
  byte cmdA0[8] = {0x02, 0xA0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  CAN0.sendMsgBuf(ID, 8, cmdA0);
  Serial.println("Enviado A0");
  readPacket(ID + 8, true);
  readPacket(ID + 8, true);
  readPacket(ID + 8, true);
  readPacket(ID + 8, true);
